...
public class MarkovModel {
   ...
   private void buildModel(int K, String sourceText) {
      ...
      int y = 0;
      int z = 0;
      first = sourceText.substring(0 , x);
      
      
      while (x + y <= sourceText.length()) {
         String empt = "";
         String kg = sourceText.substring (y, x + y);
         if (!model.containsKey(kg)) {
            int j = x;
            while (z + j < sourceText.length()) {
               String g = sourceText.substring (z, z + j);
               if (z + x >= sourceText.length()) {
                  empt += '\u0000';
               }
               if (kg.equals(g)) {
   ...
   public String getRandomKgram() {
      ...
      for (String element : model.keySet()) {
         if (index == count) {
            return element;
         }
         count++;
